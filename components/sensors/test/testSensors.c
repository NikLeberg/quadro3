 ...
 *
 */
TEST_CASE("assumptionsAreValid", "[sensors][qemu]") {
    TEST_ASSERT_LESS_OR_EQUAL_UINT32(sizeof(uint32_t),
                                     sizeof(0x1 << SENSORS_MAX));
}
 ...
 *
 */
TEST_CASE("taskStartedAfterStart", "[sensors][qemu]") {
 ...
 *
 */
TEST_CASE("restartDoesNotCreateAdditionalTask", "[sensors][qemu]") {
    uint32_t numOfTasks = uxTaskGetNumberOfTasks();
 ...
 *
 */
TEST_CASE("taskStoppedAfterStop", "[sensors][qemu]") {
    uint32_t numOfTasks = uxTaskGetNumberOfTasks();
    sensorsStop();
...
TEST_CASE("registrationForIntervalBasedSensorWorks", "[sensors][qemu]") {
    TEST_ASSERT_FALSE(sensorsRegister(SENSORS_ORIENTATION,
                                      dummyIntervalSensorCallback,
                                      (void *)0xDEADBEEF, interval));
    TickType_t startTicks = xTaskGetTickCount();
    sensorsStart();
    vTaskDelay(interval + intervalJitter);
    TEST_ASSERT_EQUAL(1, dummyIntervalSensorCallbackCallNum);
    TEST_ASSERT_LESS_THAN(interval + intervalJitter,
                          dummyIntervalSensorCallbackCalledAfter - startTicks);
    TEST_ASSERT_GREATER_THAN(interval - intervalJitter,
                             dummyIntervalSensorCallbackCalledAfter -
                                 startTicks);
...
TEST_CASE("registrationForEventBasedSensorWorks", "[sensors][qemu]") {
    TEST_ASSERT_FALSE(sensorsRegister(
        SENSORS_ORIENTATION, dummyEventSensorCallback, (void *)0xBEEFDEAD, 0));
    sensorsStart();
    ...
    TEST_ASSERT_LESS_OR_EQUAL(eventJitter,
                              dummyEventSensorCallbackCalledAfter - startTicks);
    sensorsRegister(SENSORS_ORIENTATION, NULL, NULL, 0);
    vTaskDelay(eventJitter);
    TEST_ASSERT_EQUAL(1, dummyEventSensorCallbackCallNum);
    sensorsStop();
...
TEST_CASE("setOfRawDataOnlyInSensorContext", "[sensors][qemu]") {
...
void dummyRawGetSet(void *cookie) {
    (void)cookie;
    sensorsData_t data = {.timestamp = dummyRawGetSetCalled ? 123456 : 0,
                          .vector = {.x = 1.2, .y = 2.3, .z = 3.4}};
    TEST_ASSERT_FALSE(sensorsSetRaw(SENSORS_ORIENTATION, &data, 0));
    dummyRawGetSetCalled = true;
}
TEST_CASE("getSetOfRawDataWorks", "[sensors][qemu]") {
    sensorsRegister(SENSORS_ORIENTATION, dummyRawGetSet, NULL, 0);
    sensorsStart();
    sensorsData_t data;
    TEST_ASSERT_TRUE(
        sensorsGetRaw(SENSORS_ORIENTATION, &data)); // noch keine Daten
    sensorsNotify(SENSORS_ORIENTATION);
    vTaskDelay(eventJitter);
    TEST_ASSERT_TRUE(sensorsGetRaw(SENSORS_ORIENTATION,
                                   &data)); // noch keine Daten, Timestamp war 0
    sensorsNotify(SENSORS_ORIENTATION);
    vTaskDelay(eventJitter);
...
} dummyOrientationData[dummyOrientationTestPoints] =
    { // w = real; x = i; y = j; z = k
...
TEST_CASE("orientationCorrect", "[sensors][qemu]") {
    ...
    for (uint32_t i = 0; i < dummyOrientationTestPoints; ++i) {
        ...
        vTaskDelay(eventJitter);
        sensorsData_t data;
        TEST_ASSERT_FALSE(sensorsGetState(SENSORS_STATE_ORIENTATION,
                                          SENSORS_ENU_LOCAL, &data));
        TEST_ASSERT_EQUAL_FLOAT(dummyOrientationData[i].expectedLocal.i,
                                data.quaternion.i);
        TEST_ASSERT_EQUAL_FLOAT(dummyOrientationData[i].expectedLocal.j,
                                data.quaternion.j);
        TEST_ASSERT_EQUAL_FLOAT(dummyOrientationData[i].expectedLocal.k,
                                data.quaternion.k);
        TEST_ASSERT_EQUAL_FLOAT(dummyOrientationData[i].expectedLocal.real,
                                data.quaternion.real);
        TEST_ASSERT_EQUAL(SENSORS_ENU_LOCAL, data.reference);
        TEST_ASSERT_FALSE(sensorsGetState(SENSORS_STATE_ORIENTATION,
                                          SENSORS_ENU_WORLD, &data));
        TEST_ASSERT_EQUAL_FLOAT(-dummyOrientationData[i].expectedLocal.i,
                                data.quaternion.i);
        TEST_ASSERT_EQUAL_FLOAT(-dummyOrientationData[i].expectedLocal.j,
                                data.quaternion.j);
        ...
        TEST_ASSERT_FALSE(
            sensorsGetState(SENSORS_STATE_EULER, SENSORS_ENU_LOCAL, &data));
        TEST_ASSERT_FLOAT_WITHIN(
            0.002, dummyOrientationData[i].expectedEulerLocal.x,
            data.vector.x); // math Winkelfunktionen sind nicht super genau
        TEST_ASSERT_FLOAT_WITHIN(
            0.002, dummyOrientationData[i].expectedEulerLocal.y, data.vector.y);
        TEST_ASSERT_FLOAT_WITHIN(
            0.002, dummyOrientationData[i].expectedEulerLocal.z, data.vector.z);
        TEST_ASSERT_EQUAL(SENSORS_ENU_LOCAL, data.reference);
        TEST_ASSERT_FALSE(
            sensorsGetState(SENSORS_STATE_EULER, SENSORS_ENU_WORLD, &data));
        TEST_ASSERT_FLOAT_WITHIN(
            0.002, dummyOrientationData[i].expectedEulerWorld.x, data.vector.x);
        TEST_ASSERT_FLOAT_WITHIN(
            0.002, dummyOrientationData[i].expectedEulerWorld.y, data.vector.y);
        TEST_ASSERT_FLOAT_WITHIN(
            0.002, dummyOrientationData[i].expectedEulerWorld.z, data.vector.z);
        TEST_ASSERT_EQUAL(SENSORS_ENU_WORLD, data.reference);
    }
    sensorsStop();
}
...
} dummyVectorRotatesData[dummyVectorRotatesTestPoints] =
    { // erstellt mit matlab quadrotate, quat2eul
...
void dummyVectorRotates(void *cookie) {
    ...
    data.quaternion =
        dummyVectorRotatesData[dummyVectorRotatesCallNum].testOrientationLocal;
    sensorsSetRaw(SENSORS_ORIENTATION, &data, 0);
    data.reference =
        dummyVectorRotatesData[dummyVectorRotatesCallNum].reference;
...
TEST_CASE("vectorRotatesCorrect", "[sensors][qemu]") {
    sensorsRegister(SENSORS_ROTATION, dummyVectorRotates, NULL, 0);
    sensorsStart();
    for (sensorsState_t s = SENSORS_STATE_ROTATION;
         s <= SENSORS_STATE_ACCELERATION; ++s) {
        for (uint32_t i = 0; i < dummyVectorRotatesTestPoints; ++i) {
            ...
            TEST_ASSERT_FLOAT_WITHIN(
                0.1, dummyVectorRotatesData[i].expectedLocal.x, data.vector.x);
            TEST_ASSERT_FLOAT_WITHIN(
                0.1, dummyVectorRotatesData[i].expectedLocal.y, data.vector.y);
            TEST_ASSERT_FLOAT_WITHIN(
                0.1, dummyVectorRotatesData[i].expectedLocal.z, data.vector.z);
            TEST_ASSERT_EQUAL(SENSORS_ENU_LOCAL, data.reference);
            TEST_ASSERT_FALSE(sensorsGetState(s, SENSORS_ENU_WORLD, &data));
            TEST_ASSERT_FLOAT_WITHIN(
                0.1, dummyVectorRotatesData[i].expectedWorld.x, data.vector.x);
            TEST_ASSERT_FLOAT_WITHIN(
                0.1, dummyVectorRotatesData[i].expectedWorld.y, data.vector.y);
            TEST_ASSERT_FLOAT_WITHIN(
                0.1, dummyVectorRotatesData[i].expectedWorld.z, data.vector.z);
            TEST_ASSERT_EQUAL(SENSORS_ENU_WORLD, data.reference);
        }
    }
    sensorsStop();
...
/**
 * @brief Stelle sicher dass System immer korrekt gestoppt wird, egal wie die
 * oberen Tests ausfallen.
 *
 */
TEST_CASE("ensureCleanStopForFollowingTests", "[sensors][qemu]") {
    sensorsStop();
}
