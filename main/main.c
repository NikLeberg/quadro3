...
void app_main(void) {
    ...
    while (true) {
        ...
        sensorsGetState(SENSORS_STATE_ORIENTATION, SENSORS_ENU_LOCAL, &state);
        ...
        sensorsGetState(SENSORS_STATE_EULER, SENSORS_ENU_WORLD, &state);
        printf("euler:\t%f\t%f\t%f\n", state.vector.x, state.vector.y, state.vector.z);
        sensorsGetState(SENSORS_STATE_ACCELERATION, SENSORS_ENU_WORLD, &state);
        printf("accel:\t%f\t%f\t%f\n", state.vector.x, state.vector.y, state.vector.z);
        sensorsGetState(SENSORS_STATE_ROTATION, SENSORS_ENU_WORLD, &state);
        printf("rotate:\t%f\t%f\t%f\n", state.vector.x, state.vector.y, state.vector.z);
    }
}
